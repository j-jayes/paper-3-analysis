...
lorenz_data <- df %>%
    ...
    mutate(
        population = row_number() / n(),
        income_cumulative = cumsum(income_incl_zero) / sum(income_incl_zero)
    ) %>%
    mutate(current_parish_treated = case_when(
        current_parish_treated == 1 ~ "Western Line parish",
        current_parish_treated == 0 ~ "Control parish",
        TRUE ~ NA_character_
    ))

# lorenz_data %>%
#     select(population, income_cumulative, current_parish_treated) %>%
#     write_rds(here::here("replication-data", "figure-5.rds"))
