...
df %>% 
  ...
  mutate(cumulative_income = cumsum(income_incl_zero), # Convert log income to actual income and cumulatively sum
         total_income = max(cumulative_income),
         percentile_rank = ntile(income_incl_zero, 100), # Assigning percentile rank
  ...
  ungroup() %>%
  ggplot(aes(x = percentile_rank, y = cumulative_share, color = as.factor(current_parish_treated))) +
    geom_abline() +
  geom_line() +
...
lorenz_data <- df %>% 
    filter(income_imputed_mean > 0) %>% 
    group_by(current_parish_treated) %>%
  arrange(income_imputed_mean) %>%
  mutate(population = row_number() / n(),
         income_cumulative = cumsum(income_imputed_mean) / sum(income_imputed_mean)) %>% 
