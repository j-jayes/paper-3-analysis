...
```{r}
electricity_jobs <- tibble(title = c(
...
hisclass_order <- tribble(
    ~hisclass_group_abb, ~order,
    "Elite", 1,
    "White collar", 2,
    "Foremen", 3,
...
df %>%
    ...
    mutate(napp_title = case_when(
        ...
        napp_title == "Electricity generators" ~ "Electricity generator\nworkers",
        TRUE ~ napp_title
    )) %>%
    mutate(treated = case_when(
        treated == "Treated" ~ "Early electrifying parishes",
        TRUE ~ "Control parishes"
    )) %>%
    mutate(
        napp_title = fct_reorder(napp_title, pct_share),
        pct_share_lab = scales::percent(pct_share, accuracy = 0.1)
    )  %>% write_rds(here::here("replication-data", "Figure-6.rds"))
...
ggsave(
    filename = here::here("figures/06-job_share_plot.jpeg"),
