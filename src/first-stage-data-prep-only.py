...
# What columns do I want in my final dataset
# I need one row per parish
# I need the following columns:
#   - ref_code
#   - parish_name
#   - population (all 3 years)
#   - area_sqkm
#   - distance (from line)
#   - num_power_stations
#   - amount_final
...
# select columns user, name, location, latitude, longitude, amount_final, source_final
power_stations_df = power_stations_df[['user', 'name', 'location', 'latitude', 'longitude', 'amount_final', 'source_final']]
# rename amount_final to amount and source_final to source
power_stations_df = power_stations_df.rename(columns={'amount_final': 'amount', 'source_final': 'source'})
...
population_df_1900_1910 = pd.read_excel(pathlib.Path('data/parishes/population_by_parish_1900_1910.xlsx'))
...
# pivot wider from year
population_df_1900_1910 = population_df_1900_1910.pivot(index='ref_code', columns='year', values='n').reset_index()

population_df_1930 = pd.read_excel(pathlib.Path('data/parishes/population_by_parish_1930.xlsx'))
# select ref_code, n
population_df_1930 = population_df_1930[['ref_code', 'n']]
# rename n to population_1930
population_df_1930 = population_df_1930.rename(columns={'n': 'population_1930'})

# join population_df_1900_1910 and population_df_1930
population_df = population_df_1900_1910.merge(population_df_1930, on='ref_code', how='left')
...
# calculate distance from line_gdf to parish midpoints
parishes['distance'] = parishes['geometry'].apply(lambda x: line_gdf.distance(x))

# read in treated parish list (editied by hand to increase the number of treated parishes to complete the line)
