...
*** results indirectory "results/regressions/" as a local
...
quietly summarize electricity_job_indirect if birth_parish_treated == 0
local mean1 = round(r(mean), 0.01)
probit electricity_job_indirect birth_parish_treated, robust
eststo Model1
estadd scalar mean_depvar = `mean1'
...
quietly summarize electricity_job_indirect if birth_parish_treated == 0
local mean2 = round(r(mean), 0.01)
probit electricity_job_indirect birth_parish_treated age age_2 female, robust
eststo Model2
estadd scalar mean_depvar = `mean2'
...
quietly summarize electricity_job_indirect if birth_parish_treated == 0
local mean3 = round(r(mean), 0.01)
probit electricity_job_indirect birth_parish_treated age age_2 female i.marital i.schooling, robust
eststo Model3
estadd scalar mean_depvar = `mean3'
...
* Output the results
esttab Model1 Model2 Model3 using `results_dir'/electricity_job_indirect_probit.tex, label replace ///
