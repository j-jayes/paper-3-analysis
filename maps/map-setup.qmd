...
Need the power lines, the location of the stations, and the type of station. Then need the Western Line parishes.
...
### Big power stations and power lines 1926 and Western Line parishes
...
ggplot() +
  ...
  geom_sf(data = parishes_subset_treated,  colour = "grey70", aes(fill = "Western Line parishes")) + 
  geom_sf_text(aes(label = name), data = hydro, size = 8, nudge_y = .35) +
  # Lines
  geom_sf(aes(colour = "Electricity lines\n1926"),
    data = map_grid %>%
  ...
  # Fills
  scale_fill_manual(values = c("Western Line parishes" = "#fc8d59"), guide = guide_legend(override.aes = list(linetype = "blank", shape = NA))) +
  scale_color_manual(
    values = c("Electricity lines\n1926" = "#000080", "Hydropower plants" = "#d7301f"),
    guide = guide_legend(override.aes = list(
      linetype = c("solid", "blank"),
...
### Big power stations and power lines 1926 and Western Line parishes and control parishes
...
ggplot() +
  ...
  # Fills
  scale_fill_manual(
    values = c("Western Line parishes" = "#fc8d59", "Control parishes" = "#fef0d9"),
...
## New control parishes and 1930 census Western Line parishes
...
# read in Western Line parishes from data/parishes/treated_parishes.xlsx
...
parishes_subset_treated_1930 <- treated_parishes %>% 
  left_join(parishes, by = "ref_code") %>% 
  st_as_sf()

parishes_subset_control_1930 <- parishes %>% 
  filter(current_parish_touching_treated == 0,
         current_parish_treated == 0,
         current_parish_distance_to_line <= 300)


figure_3_data <- list(hydro = hydro, counties = counties, parishes_subset_treated = parishes_subset_treated, parishes_subset_control = parishes_subset_control, map_grid = map_grid, parishes_subset_treated_1930 = parishes_subset_treated_1930, parishes_subset_control_1930 = parishes_subset_control_1930)

# figure_3_data %>% write_rds(here::here("replication-data", "figure-3.rds"))
...
ggplot() +
    ...
    # Western Line parishes
    geom_sf(data = parishes_subset_treated_1930, colour = "grey70", aes(fill = "Western Line parishes")) +
    geom_sf_text(aes(label = name), data = hydro, size = 8, nudge_y = .35) +
    # Lines
    geom_sf(aes(colour = "Electricity lines\n1926"),
        data = map_grid %>%
...
Testing again with 1930 data for control parishes
...
ggplot() +
    ...
    geom_sf(data = parishes_subset_treated_1930, colour = "grey70", aes(fill = "Western Line parishes")) +
    # control parishes
    geom_sf(data = parishes_subset_control_1930, colour = "grey70", aes(fill = "Control parishes")) +
    geom_sf_text(aes(label = name), data = hydro, size = 8, nudge_y = .35) +
    # Lines
    geom_sf(aes(colour = "Electricity lines\n1926"),
        data = map_grid %>%
    ...
    scale_fill_manual(values = c(
        "Western Line parishes" = "#fc8d59",
        "Control parishes" = "#fef0d9"
    ), guide = guide_legend(override.aes = list(linetype = "blank", shape = NA))) +
    scale_color_manual(
        values = c("Electricity lines\n1926" = "#000080", "Hydropower plants" = "#d7301f"),
        guide = guide_legend(override.aes = list(
...
# Now testing for probability of inclusion in treatment
...
ggplot() +
    ...
    # Western Line parishes
    geom_sf(data = figure_3_data$parishes_subset_treated, colour = "grey70", aes(fill = "Western Line parishes")) +
    # Control parishes
